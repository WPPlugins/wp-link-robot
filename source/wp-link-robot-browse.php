...
-->
  ...
  Category filter: <select name="category">
	...
	<?php
	global $wpdb;
  $aaCategories = $wpdb->get_results( "SELECT id,title FROM `{$wpdb->prefix}eblex_categories` WHERE parent = \"\" ORDER BY `zindex` DESC" );
  $aSubCats = array();
  foreach($aaCategories as $aCategory){
    ...
    if ($aCategory->id != 0){
      $aaSubCategories = $wpdb->get_results( "SELECT id,title FROM `{$wpdb->prefix}eblex_categories` WHERE parent = \"{$aCategory->id}\" ORDER BY `zindex` DESC" );
...
<?php
  ...
  if (isset($_POST['category']) AND $_POST['category'] != "-1" )
   ...
   //pagination
	$iLinkscount = $wpdb->get_var( "SELECT count(*) FROM `{$wpdb->prefix}eblex_links`" . $sWhere . " ORDER BY `zindex` DESC" );
   $iLinks_per_page = 20;
   ...
   //end of pagination
	...
	$links = $wpdb->get_results( "SELECT * FROM `{$wpdb->prefix}eblex_links`" . $sWhere . " ORDER BY `zindex` DESC LIMIT ".$iFrom_link.",".$iLinks_per_page );
