...
<!--	Addition	04/07/2008	mVicenik	Foliovision	-->
	...
	<tr valign="top">
	  <td><select name="category" id="category">
	    ...
	    <?php
	    $cat = $wpdb->get_results("SELECT * FROM `$eblex_categories` WHERE `parent`='' ORDER BY `zindex` DESC, `title`");
		...
		foreach ($cat as $row)
	  	{
   		if ($row->id != "0")
   		{
      		...
      		<?php
      		$subcat=$wpdb->get_results("SELECT * FROM `$eblex_categories` WHERE `parent`='".$row->id."' ORDER BY `zindex` DESC, `title`");
...
if ($_POST['timeout']!="")
{
	...
	if (is_numeric($timeout))
	{
		...
		if(isset($_POST['checkbox2'])) {
			$numberoflinks = $wpdb->get_var("SELECT count(*) FROM `$eblex_links` WHERE `status`='2' ORDER BY `zindex` DESC, `title`");
			$numberofreciprociallinks = $wpdb->get_var("SELECT count(*) FROM `$eblex_links` WHERE `status`='2' AND `reciprocalurl`!='' AND `nonreciprocal`!='1' ORDER BY `zindex` DESC, `title`");
		}
		else {
			if(isset($_POST['checkbox'])) {
				...
				if ($_POST['category'] == "0" || $_POST['category'] == "") {
					$numberoflinks = $wpdb->get_var("SELECT count(*) FROM `$eblex_links` WHERE `status`='1' ORDER BY `zindex` DESC, `title`");
					$numberofreciprociallinks = $wpdb->get_var("SELECT count(*) FROM `$eblex_links` WHERE `status`='1' AND `reciprocalurl`!='' AND `nonreciprocal`!='1' ORDER BY `zindex` DESC, `title`");
				}
				else {
					$cat = $wpdb->get_results("SELECT * FROM `$eblex_categories` WHERE `id`='$l_category' OR `parent`='$l_category' ORDER BY `zindex` DESC, `title`");
					foreach ($cat as $row) {
						$numberoflinks += $wpdb->get_var("SELECT count(*) FROM `$eblex_links` WHERE `status`='1' AND `category`='$row->id' ORDER BY `zindex` DESC, `title`");
						$numberofreciprociallinks += $wpdb->get_var("SELECT count(*) FROM `$eblex_links` WHERE `status`='1' AND `category`='$row->id' AND `reciprocalurl`!='' AND `nonreciprocal`!='1' ORDER BY `zindex` DESC, `title`");
					}
				}
			}
			else {
				///	Modification	04/07/2008	mVicenik	Foliovision
				$numberoflinks = $wpdb->get_var("SELECT count(*) FROM `$eblex_links` WHERE `status`='1' AND `category`='$l_category' ORDER BY `zindex` DESC, `title`");
				//$numberoflinks = $wpdb->get_var("SELECT count(*) FROM `$eblex_links` WHERE `status`='1' ORDER BY `zindex` DESC");
				///	end of modification
				///	Addition	04/07/2008	mVicenik	Foliovision
				$numberofreciprociallinks = $wpdb->get_var("SELECT count(*) FROM `$eblex_links` WHERE `status`='1' AND `category`='$l_category' AND `reciprocalurl`!='' AND `nonreciprocal`!='1' ORDER BY `zindex` DESC");
				///	end of addition
			}
		}
		///	end of addition
		...
		if ($numberoflinks != "0")
		{
			...
			if(isset($_POST['checkbox2'])) {	//	clean trash only
				$lnk = $wpdb->get_results("SELECT * FROM `$eblex_links` WHERE `status`='2' ORDER BY `zindex` DESC, `title`");
				////
			echo '<table class="widefat">	
				...
				if(isset($_POST['checkbox'])) {	//	clean recursively	
					if ($_POST['category'] == "0" || $_POST['category'] == "") {	//	from Root
						$lnk = $wpdb->get_results("SELECT * FROM `$eblex_links` WHERE `status`='1' ORDER BY `zindex` DESC, `title`");
						////
						echo '<table class="widefat">	
                  <thead>
                  <tr><th></th><th>Title</th><th width="15%">Info</th><th width="20%">Reciprocal URL</th><th>Status</th><th>Response*</th><th>Actions</th></tr>
                  ...
                  <tbody>';
				  	...
				  	echo '</tbody>
						...
						$checkcount = 0;
						foreach ($cat as $row) {
							$lnk = $wpdb->get_results("SELECT * FROM `$eblex_links` WHERE `status`='1' AND `category`='$row->id' ORDER BY `zindex` DESC, `title`");
							////
						echo '<table class="widefat">	
                  <thead>
                  <tr><th></th><th>Title</th><th width="15%">Info</th><th width="20%">Reciprocal URL</th><th>Status</th><th>Response*</th><th>Actions</th></tr>
                  ...
                  <tbody>';
					...
					///	Modification	04/07/2008	mVicenik	Foliovision
					$lnk = $wpdb->get_results("SELECT * FROM `$eblex_links` WHERE `status`='1' AND `category`='$l_category' ORDER BY `zindex` DESC");
