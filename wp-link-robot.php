...
//		$newrules[$pageslugname . '/([0-9]{1,})/?$'] = 'index.php?&pagename=' . $pageslugname . '&pg=' . $match_form;
		...
		$newrules[$pageslugname . '/([a-z\-]*)/([a-z\-]*)/(\d*)/?$'] = 'index.php?&fv_pagename=' . $pageslugname . '&link_robot_category=$matches[1]&link_robot_subcategory=$matches[2]&link_robot_page=$matches[3]';
...
function wp_link_robot_add_new_link() {
  ...
  if($_POST['submit']){
    ...
    $sOptions = '';
    $aaCategories = $wpdb->get_results( "SELECT id,title FROM `{$wpdb->prefix}eblex_categories` WHERE parent = \"\"  AND visible='1' ORDER BY `zindex` DESC" );
    foreach($aaCategories as $aCategory){
      if(strlen($aCategory->id) > 1){
        $sOptions .= "<option value=\"{$aCategory->id}\">{$aCategory->title}</option>\n";
        //if ($aCategory->id != 0){
          $aaSubCategories = $wpdb->get_results( "SELECT id,title FROM `{$wpdb->prefix}eblex_categories` WHERE parent = \"{$aCategory->id}\" AND visible='1' ORDER BY `zindex` DESC" );
          if($aaSubCategories)
